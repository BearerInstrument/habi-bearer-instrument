...
**This is:** A safety-focused formal verification project for a disconnection-tolerant bearer-instrument ledger. It includes a TLA+ model that found a genuine double-spend vulnerability in a naive disconnect/reconnect design, an independent Coq mechanization of a safety invariant (10 machine-checked theorems), and a Rust implementation validated by 46 automated tests, including live reproduction across three separate networked processes over real TCP. This repository is submitted as supporting technical evidence for the HABI white paper under Army xTech|Search 10 and reflects prior work submitted under NRL Long-Range BAA N00173-24-S-BA01, Topic 55-24-02.

**This is not:** A production-ready or hardened system, and not currently a proven one. As of 2026-09-15, the per-link model's core safety invariant (`PairwiseSyncedNoDoubleSpend`) does NOT hold exhaustively -- TLC finds a real violation in every recorded run (see EVIDENCE.md for the root cause and current status). Conversion Day, the intended mitigation, is confirmed by a passing Rust unit test to detect this specific class of staleness after the fact, but no exhaustive model of the combined per-link-plus-Conversion-Day system exists yet, and detection is not the same guarantee as prevention. No liveness claim is made about disconnected nodes ("stragglers"). The `seed` mechanism is bootstrap-only and does not model real-world issuance. This is a transliteration of the TLA+-verified design into Rust, not an independent re-verification of the Rust code itself -- and per the above, the design's per-link safety property is not itself currently proven.
